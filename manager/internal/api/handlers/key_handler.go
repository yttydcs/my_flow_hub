...
func (h *KeyHandler) HandleListKeys(w http.ResponseWriter, r *http.Request) {
	...
	req := protocol.BaseMessage{ID: uuid.New().String(), Type: "key_list", Payload: map[string]interface{}{"userKey": token}}
...
func (h *KeyHandler) HandleDeleteKey(w http.ResponseWriter, r *http.Request) {
	...
	req := protocol.BaseMessage{ID: uuid.New().String(), Type: "key_delete", Payload: map[string]interface{}{"userKey": token, "id": body.ID}}
	resp, err := h.hubClient.SendRequest(req, 5*time.Second)
	if err != nil {
		h.writeError(w, http.StatusInternalServerError, err.Error())
		return
...
func (h *KeyHandler) HandleKeyDevices(w http.ResponseWriter, r *http.Request) {
	...
	req := protocol.BaseMessage{ID: uuid.New().String(), Type: "key_devices", Payload: map[string]interface{}{"userKey": token}}
	resp, err := h.hubClient.SendRequest(req, 5*time.Second)
	if err != nil {
		h.writeError(w, http.StatusInternalServerError, err.Error())
		return
